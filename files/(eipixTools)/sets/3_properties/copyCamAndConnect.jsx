...
(function copyCamAndConnect(thisObj) {
    ...
    var selectedCameraComp;
    var electedCameraCompName;
    //
    ...
    function copyCamAndConnect_doGetComp() {
        ...
        if (activeItem === null) {
            ...
            if (!(activeItem.selectedLayers.length == 1) || !(selectedLayer.source instanceof CompItem)) {
                alert(copyCamAndConnect_localize(copyCamAndConnectData.strErrSelectMultiComp));
                return;
            } else {
                gotCompositionSwitch = true;
                selectedComposition = selectedLayer.source;
                ccacPal.grp.opts.getComp.compName.text = selectedLayer.name;
            }
    ...
    function copyCamAndConnect_doExecute() {
        if ((gotCameraSwitch == true) && (gotCompositionSwitch == true)) {
            ...
            instanceCam.orientation.expression = "C = comp(\"" + selectedCameraCompName + "\").layer(" + "\"" + selectedCameraName + "\"" + ");\ru = C.toWorldVec([1,0,0]);\rv = C.toWorldVec([0,1,0]);\rw = C.toWorldVec([0,0,1]);" + "sinb = clamp(w[0],-1,1);\rb = Math.asin(sinb/thisComp.pixelAspect);\rcosb = Math.cos(b);\rif (Math.abs(cosb) > .0005) {\rc = -Math.atan2(v[0],u[0]);\r" + "a = -Math.atan2(w[1],w[2]);\r} else {\ra = Math.atan2(u[1],v[1]);\rc = 0;\r}\r[radians_to_degrees(a),radians_to_degrees(b),radians_to_degrees(c)];";
